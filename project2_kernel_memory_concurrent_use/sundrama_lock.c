...
int *number; //store all numbers corresponding with users' input
...
static void compute(void *count){
    ...
    while(true){
        ...
        while(number[cur] == 0 && cur*4 <= upper_bound){
            cur++;
        }
        spin_unlock(&lock);
        if(cur*4 > upper_bound) {
            return;
        }
        
        //execute sieve of Sundaram algorithm
        i = current_pos;
            for(j = i; j <= upper_bound/4; j++){
                //ctoss-out numbers
                if(j+i+2*i*j<=upper_bound){
                    spin_lock(&lock);
                    number[j+i+2*i*j] = 0;
                    spin_unlock(&lock);
...
static void showinfo(void) {
    ...
    //initialize a tempt array for calculate
    tmp = (int *) kmalloc(sizeof(int)*(upper_bound+1), GFP_KERNEL);
	if (IS_ERR(tmp)) {
		printk(KERN_WARNING "allocate tmp array for number failed!\n");
		return;
	}    
    ...
    //update number
    for(index=1; index<=upper_bound; index++){
        if(number[index]!=0){
            i = index*2 + 1;
            if(i <= upper_bound){
                tmp[i] = i;
    ...
    for(index = 1; index <= (upper_bound); index++){
        if(number[index] != 0){
            printk(KERN_CONT "%*d,",7, number[index]);
            iter++;
            if(index != 0 && iter%8 == 0){
                printk(KERN_CONT "\n");
            }
    ...
    printk(KERN_WARNING "Time from the initialization time stamp to the time stamp taken at the first barrier:%ld s %ld ns\n",arrive_time.tv_sec, arrive_time.tv_nsec);
    ...
    printk(KERN_WARNING "Time from the time stamp taken at the first barrier to the time stamp taken at the second barrier: %ld s %ld ns\n",compute_time.tv_sec,compute_time.tv_nsec);
    //completion time (initialization time plus prime computation time) 
    printk(KERN_WARNING "Total time for module insmod to compute completion: %ld s %ld ns\n",module_time.tv_sec, module_time.tv_nsec);
...
static int simple_init(void){
    ...
    number = (int *) kmalloc(sizeof(int)*(upper_bound+1), GFP_KERNEL);
	if (IS_ERR(number)) {
