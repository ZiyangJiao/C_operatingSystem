...
static void compute(void *count){
    ...
    while(true){
        ...
        while(atomic_read(&number[cur]) == 0 && cur*4 <= upper_bound){
            cur++;
        }
        spin_unlock(&lock);
...
static void showinfo(void) {
    ...
    for(index=1; index<=upper_bound; index++){
        atomic_set(&number[index],tmp[index]);
    }
    atomic_set(&number[2],2);
...
static int simple_init(void){
    ...
    atomic_set(&number[0], 0);
    for(index = 1; index <= upper_bound; index++){
        atomic_set(&number[index], index);
    }
