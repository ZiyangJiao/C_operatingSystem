...
int main( int argc, char* argv[] ){
	...
	int i;
 	struct timespec tmp,start,end,max,min,mean,sum;
	for(i = 1; atoi(argv[2]) - i >= 0; i++){
		...
		if(i == 1){
			min.tv_sec = tmp.tv_sec;
			min.tv_nsec = tmp.tv_nsec;
			sum.tv_sec = 0;
			sum.tv_nsec = 0;
		}
		sum.tv_sec += tmp.tv_sec;
		sum.tv_nsec += tmp.tv_nsec;
		mean.tv_sec = sum.tv_sec / i;
		mean.tv_nsec = sum.tv_nsec / i;
		if(tmp.tv_sec >= max.tv_sec){
			max.tv_sec = tmp.tv_sec;
			if(tmp.tv_nsec > max.tv_nsec){
				max.tv_nsec = tmp.tv_nsec;
			}
		}
		if(tmp.tv_sec <= min.tv_sec){
			min.tv_sec = tmp.tv_sec;
			if(tmp.tv_nsec < min.tv_nsec){
				min.tv_nsec = tmp.tv_nsec;
			}
		}
		printf("It is the %d iterations with %ld seconds %ld nanoseconds. Up to now, \nthe max:%ld seconds %ld nanoseconds; \nthe min:%ld seconds %ld nanoseconds; \nmean:%ld seconds %ld nanoseconds\n"
		,i,tmp.tv_sec,tmp.tv_nsec,max.tv_sec,max.tv_nsec,min.tv_sec,min.tv_nsec,mean.tv_sec,mean.tv_nsec);
